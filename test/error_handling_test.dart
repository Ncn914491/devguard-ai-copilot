...
import 'package:devguard_ai_copilot/core/monitoring/system_health_monitor.dart';
...
void main() {
  group('Error Handling Tests', () {
    ...
    group('Error Recovery', () {
      ...
      test('should provide appropriate suggested actions', () async {
        ...
        expect(result.suggestedActions,
    ...
    group('User-Friendly Messages', () {
      test('should generate user-friendly messages for network errors',
          () async {
      ...
      test('should generate user-friendly messages for database errors',
          () async {
    ...
    group('Error Statistics', () {
      test('should track error statistics', () async {
        ...
        await errorHandler.handleError(Exception('critical error'),
      ...
      test('should calculate system health based on errors', () async {
        ...
        expect(stats.systemHealth,
            isIn([SystemHealth.degraded, SystemHealth.critical]));
      });
    });
  });
  ...
  group('Integration Tests', () {
    test('should integrate error handling with health monitoring', () async {
      ...
      expect(healthStatus.status,
          isIn([HealthStatus.warning, HealthStatus.critical]));
    });
  ...
  group('Performance Tests', () {
    ...
    test('should not leak memory during extended operation', () async {
      ...
      for (int batch = 0; batch < 10; batch++) {
        for (int i = 0; i < 10; i++) {
          await errorHandler
              .handleError(Exception('memory test error $batch-$i'));
        }
      ...
      final result =
          await errorHandler.handleError(Exception('final test error'));
      expect(result.handled, isTrue);
    });
  });
}
