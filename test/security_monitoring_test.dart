...
void main() {
  group('Enhanced Security Monitoring', () {
    ...
    group('Honeytoken Breach Detection', () {
      test('should detect honeytoken access and create critical alert',
          () async {
        // Arrange
      ...
      test(
          'should generate appropriate AI explanation for different honeytoken types',
          () async {
        // Test different honeytoken types
        final honeytokenTypes = [
          'credit_card',
          'ssn',
          'api_key',
          'password_hash',
    ...
    group('Data Export Anomaly Detection', () {
      ...
      test('should detect off-hours database access', () async {
        ...
        await securityMonitor.simulateOffHoursAccess(
            8, 23); // 8 queries at 11 PM
      ...
      test('should calculate correct severity based on volume increase',
          () async {
        ...
        for (final testCase in testCases) {
          await securityMonitor.simulateDataExportAnomaly(
              testCase['current'] as int, testCase['baseline'] as int);
    ...
    group('Configuration Drift Detection', () {
      test('should detect configuration file changes', () async {
        ...
        await securityMonitor.simulateConfigurationDrift('config/security.json',
            'credential_modification', 'abc123def456', 'xyz789uvw012');
        ...
        expect(configAlert.severity,
            equals('critical')); // credential_modification
      ...
      test('should assign correct severity based on change type', () async {
        final testCases = [
          {
            'changeType': 'credential_modification',
            'expectedSeverity': 'critical'
          },
          {
        ...
        for (final testCase in testCases) {
          await securityMonitor.simulateConfigurationDrift('config/test.json',
              testCase['changeType'] as String, 'hash1', 'hash2');
      ...
      test('should suggest rollback for critical configuration changes',
          () async {
        final criticalChanges = [
        ...
        for (final changeType in criticalChanges) {
          await securityMonitor.simulateConfigurationDrift(
              'config/security.json', changeType, 'hash1', 'hash2');
      ...
      test('should assign correct severity based on attempt count', () async {
        ...
        for (final testCase in testCases) {
          await securityMonitor.simulateLoginFlood(
              '192.168.1.200', testCase['attempts'] as int);
    ...
    group('AI Explanation Generation', () {
      test('should generate appropriate explanations for different alert types',
          () async {
        ...
        await securityMonitor.simulateConfigurationDrift(
            'config/app.json', 'privilege_escalation', 'hash1', 'hash2');
        alerts = await alertService.getRecentAlerts(limit: 1);
        alert = alerts.first;
      ...
      test('should provide actionable recommendations in explanations',
          () async {
    ...
    group('Integration with Rollback System', () {
      test('should suggest rollback for critical security events', () async {
        final criticalEvents = [
          ...
          () => securityMonitor.simulateDataExportAnomaly(
              60, 10), // 500% increase
    ...
    group('Alert Evidence Collection', () {
      test('should collect comprehensive evidence for each alert type',
          () async {
        ...
        await securityMonitor.simulateConfigurationDrift('config/test.json',
            'network_configuration', 'original_hash', 'current_hash');
        alerts = await alertService.getRecentAlerts(limit: 1);
        alert = alerts.first;
