...
class DemoDataGenerator {
  ...
  Future<void> _generateTeamMembers() async {
    ...
    final teamMembers = [
      ...
      TeamMember(
        ...
        status: 'bench',
        assignments: [],
        expertise: ['Vue.js', 'TypeScript', 'UI/UX Design', 'Frontend Architecture'],
        workload: 0,
  ...
  Future<void> _generateSpecifications() async {
    ...
    final specifications = [
      Specification(
        id: _uuid.v4(),
        originalInput: 'Implement OAuth 2.0 authentication with Google and GitHub providers, including JWT token management and refresh token rotation',
        aiInterpretation: 'This specification requires implementing a comprehensive OAuth 2.0 authentication system supporting multiple providers (Google, GitHub) with secure JWT token management, automatic refresh token rotation, and proper session handling.',
        suggestedBranchName: 'feature/oauth2-authentication',
        suggestedCommitMessage: 'feat: implement OAuth 2.0 authentication with Google and GitHub providers\n\n- Add OAuth 2.0 flow implementation\n- Integrate Google and GitHub authentication providers\n- Implement JWT token management with refresh rotation\n- Add secure session handling and user profile management',
        status: 'approved',
      ...
      Specification(
        id: _uuid.v4(),
        originalInput: 'Create a real-time notification system for security alerts with email, Slack, and in-app notifications',
        aiInterpretation: 'This specification involves building a multi-channel notification system that can deliver security alerts through email, Slack integration, and in-app notifications with real-time updates using WebSockets.',
        suggestedBranchName: 'feature/notification-system',
        suggestedCommitMessage: 'feat: implement real-time notification system for security alerts\n\n- Add multi-channel notification support (email, Slack, in-app)\n- Implement WebSocket-based real-time notifications\n- Create notification preferences and filtering\n- Add notification history and acknowledgment tracking',
        status: 'draft',
        assignedTo: null,
      ...
      Specification(
        id: _uuid.v4(),
        originalInput: 'Build a comprehensive audit dashboard with filtering, search, and export capabilities for compliance reporting',
        aiInterpretation: 'This specification requires creating an advanced audit dashboard with sophisticated filtering options, full-text search capabilities, and export functionality to support compliance reporting requirements.',
        suggestedBranchName: 'feature/audit-dashboard',
        suggestedCommitMessage: 'feat: build comprehensive audit dashboard with advanced features\n\n- Implement advanced filtering and search functionality\n- Add export capabilities (PDF, CSV, JSON)\n- Create compliance reporting templates\n- Add audit trail visualization and analytics',
        status: 'approved',
      ...
      Specification(
        id: _uuid.v4(),
        originalInput: 'Implement automated security scanning for code repositories with vulnerability detection and remediation suggestions',
        aiInterpretation: 'This specification involves integrating automated security scanning tools to analyze code repositories, detect vulnerabilities, and provide AI-powered remediation suggestions with integration into the development workflow.',
        suggestedBranchName: 'feature/security-scanning',
        suggestedCommitMessage: 'feat: implement automated security scanning with vulnerability detection\n\n- Integrate security scanning tools (SAST, DAST, dependency scanning)\n- Add vulnerability detection and classification\n- Implement AI-powered remediation suggestions\n- Create security scanning reports and dashboards',
        status: 'in_progress',
  ...
  Future<void> _generateTasks() async {
    ...
    final tasks = [
      Task(
        ...
        description: 'Set up Google OAuth 2.0 authentication provider with proper scopes and user profile retrieval',
        type: 'feature',
        priority: 'high',
        status: 'in_progress',
      ...
      Task(
        ...
        description: 'Create architectural design for multi-channel notification system with scalability considerations',
        type: 'feature',
        priority: 'medium',
        status: 'pending',
      ...
      Task(
        ...
        description: 'Investigate and resolve false positive security alerts in honeytoken monitoring system',
        type: 'bug',
        priority: 'high',
        status: 'review',
      ...
      Task(
        ...
        description: 'Improve performance of audit log queries and add proper indexing for large datasets',
        type: 'feature',
        priority: 'medium',
        status: 'completed',
      ...
      Task(
        ...
        description: 'Create automated rollback mechanism with health checks and verification steps',
        type: 'deployment',
        priority: 'critical',
        status: 'blocked',
        assigneeId: 'alice-johnson-id',
  ...
  Future<void> _generateDeployments() async {
    ...
    final deployments = [
      Deployment(
        ...
        logs: ['Deployment started', 'Tests passed', 'Health checks passed', 'Deployment completed'],
      ),
      Deployment(
        id: _uuid.v4(),
        environment: 'staging',
      ...
      Deployment(
        ...
        logs: ['Deployment started', 'Health checks failed', 'Initiating rollback', 'Rollback completed'],
      ),
    ];
  ...
  Future<void> _generateSecurityAlerts() async {
    ...
    final alerts = [
      SecurityAlert(
        ...
        description: 'Unauthorized access to honeytoken credit card data detected from IP 192.168.1.100',
        aiExplanation: 'A honeytoken containing fake credit card information has been accessed, indicating a potential data breach attempt. The access originated from an internal IP address, suggesting possible insider threat or compromised internal system. Immediate investigation is recommended.',
        ...
        evidence: jsonEncode({
          ...
          'access_time': DateTime.now().subtract(const Duration(hours: 2)).toIso8601String(),
          'query_pattern': 'SELECT * FROM users WHERE credit_card LIKE \'4111%\'',
        }),
      ),
      SecurityAlert(
        id: _uuid.v4(),
        ...
        description: 'Detected abnormal outbound network connections to suspicious IP addresses',
        aiExplanation: 'Network monitoring has identified unusual outbound traffic patterns to IP addresses associated with known command and control servers. The traffic volume is 300% above baseline and includes encrypted communications on non-standard ports.',
        status: 'new',
      ...
      SecurityAlert(
        ...
        aiExplanation: 'Authentication monitoring has detected 15 consecutive failed login attempts for the admin account from IP 203.0.113.50 within a 5-minute window. This pattern is consistent with a brute force attack. The account has been temporarily locked as a precautionary measure.',
        status: 'resolved',
        detectedAt: DateTime.now().subtract(const Duration(hours: 6)),
        resolvedAt: DateTime.now().subtract(const Duration(hours: 5, minutes: 30)),
        rollbackSuggested: false,
        evidence: jsonEncode({
      ...
      SecurityAlert(
        ...
        description: 'Unexpected modification detected in application configuration file',
        aiExplanation: 'File integrity monitoring has detected an unauthorized modification to the main application configuration file. The change appears to be related to logging configuration and may have been made during routine maintenance. However, the modification was not logged through proper change management procedures.',
  ...
  Future<void> _generateAuditLogs() async {
    ...
    final userIds = ['alice-johnson-id', 'bob-chen-id', 'carol-martinez-id', 'david-kim-id'];
    final descriptions = {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      final userId = _random.nextBool() ? userIds[_random.nextInt(userIds.length)] : null;
  ...
  String _generateAIReasoning(String actionType) {
    final reasonings = {
      'user_login': 'User authentication successful, session established with proper security protocols',
      'specification_created': 'Natural language specification processed and converted to structured development tasks',
      'deployment_started': 'Automated deployment pipeline triggered based on approved specification',
      'security_alert_generated': 'Security monitoring detected anomalous behavior requiring investigation',
      'task_assigned': 'AI-suggested task assignment based on team member expertise and current workload',
      'configuration_changed': 'System configuration updated to improve security and performance',
      'backup_created': 'Automated backup created as part of data protection strategy',
      'rollback_initiated': 'Deployment rollback triggered due to health check failures',
      'team_member_added': 'New team member onboarded with appropriate role and permissions',
      'copilot_command_executed': 'AI copilot processed user command and provided contextual assistance',
    };
    
    return reasonings[actionType] ?? 'System action performed as part of normal operations';
  }
  ...
  Map<String, dynamic> _generateContextData(String actionType) {
    switch (actionType) {
      ...
      case 'deployment_started':
        return {
          'environment': ['development', 'staging', 'production'][_random.nextInt(3)],
          'version': '1.2.${_random.nextInt(10)}',
          'pipeline_id': _uuid.v4(),
        };
      case 'security_alert_generated':
        return {
          'alert_type': ['database_breach', 'network_anomaly', 'auth_flood'][_random.nextInt(3)],
