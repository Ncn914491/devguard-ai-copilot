-- DevGuard AI Copilot - Row-Level Security Policies
...
CREATE POLICY "users_update_own_profile" ON users
  FOR UPDATE USING (
    auth.uid() = id AND 
    (OLD.role = NEW.role OR is_admin()) -- Role can only be changed by admin
  );
...
BEGIN
  ...
  IF NEW.role = 'admin' AND OLD.role != 'admin' AND NOT is_admin() THEN
    RAISE EXCEPTION 'Only admins can grant admin privileges';
  END IF;
  
  -- Prevent non-leads from setting lead_developer role
  IF NEW.role = 'lead_developer' AND OLD.role != 'lead_developer' AND NOT is_lead_or_admin() THEN
...
-- Apply privilege escalation prevention trigger
...
BEGIN
  ...
  INSERT INTO users (email, name, role) VALUES 
    ('admin@test.com', 'Admin User', 'admin') RETURNING id INTO admin_user_id;
  INSERT INTO users (email, name, role) VALUES 
    ('lead@test.com', 'Lead User', 'lead_developer') RETURNING id INTO lead_user_id;
  INSERT INTO users (email, name, role) VALUES 
    ('dev@test.com', 'Dev User', 'developer') RETURNING id INTO dev_user_id;
  INSERT INTO users (email, name, role) VALUES 
    ('viewer@test.com', 'Viewer User', 'viewer') RETURNING id INTO viewer_user_id;
  
  -- Test 1: Admin can read all users
  BEGIN
    PERFORM * FROM users WHERE id = admin_user_id;
    RETURN QUERY SELECT 'Admin can read users'::TEXT, TRUE, NULL::TEXT;
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'Admin can read users'::TEXT, FALSE, SQLERRM;
  END;
  
  -- Test 2: Developer can read team members
  BEGIN
    PERFORM * FROM team_members LIMIT 1;
    RETURN QUERY SELECT 'Developer can read team members'::TEXT, TRUE, NULL::TEXT;
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'Developer can read team members'::TEXT, FALSE, SQLERRM;
  END;
  
  -- Test 3: Viewer has limited access
  BEGIN
    PERFORM * FROM tasks WHERE confidentiality_level = 'public' LIMIT 1;
    RETURN QUERY SELECT 'Viewer can read public tasks'::TEXT, TRUE, NULL::TEXT;
  EXCEPTION WHEN OTHERS THEN
    RETURN QUERY SELECT 'Viewer can read public tasks'::TEXT, FALSE, SQLERRM;
  END;
...
COMMENT ON FUNCTION prevent_privilege_escalation IS 'Trigger function to prevent unauthorized privilege escalation';
COMMENT ON FUNCTION test_rls_policies IS 'Function to test RLS policies functionality';
